            ...
            <a-table
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.dataIndex === 'name'">
                        <a @click="deploymentInfo(record)"   style="font-weight: bold;"  >{{ record.metadata.name }}</a>
                    </template>
                    ...
                    <template v-if="column.key === 'action'">
                        <c-button class="deployment-button" type="cyan" icon="form-outlined" @click="getDeploymentDetail(record)">YML</c-button>
                        <c-button style="margin-bottom:5px;" class="deployment-button" type="warning" icon="delete-outlined" @click="showConfirm('删除', record.metadata.name, delDeployment)">删除</c-button>
                        <c-button class="deployment-button" type="success" icon="block-outlined" @click="handleScale(record)">扩容</c-button>
                        <c-button class="deployment-button" type="purple" icon="retweet-outlined" @click="showConfirm('重启', record.metadata.name, restartDeployment)">重启</c-button>
                    </template>
                </template>
            </a-table>
        </a-card>
...
export default({
    ...
    setup() {
        ...
        function deploymentInfo(e) {
            localStorage.setItem('deploymentname', e.metadata.name)
            localStorage.setItem('deployment_label', "app="+e.metadata.labels["app"])
            router.push("/workload/deployment/info?deploymentname="+e.metadata.name);
        }
        //EOF
