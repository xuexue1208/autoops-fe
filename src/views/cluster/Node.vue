       ...
       <a-card :bodyStyle="{padding: '10px'}">
            <a-table
                ...
                <template #bodyCell="{ column, record }">
                    ...
                    <template v-if="column.dataIndex === 'schedulable'">
                        <template v-if="record.spec.unschedulable == true">
                            <a-tag color="#FF6600">Disable</a-tag>
                        </template>
                        <template v-if="record.spec.unschedulable != true">
                            <a-tag color="#33CC00">Enable</a-tag>
                        </template>
                    ...
                    <template v-if="column.key === 'action'">
                        <c-button style="margin-bottom:5px;" class="node-button" type="cyan" icon="form-outlined" @click="getNodeDetail(record)">YML</c-button>
                        <template v-if="record.spec.unschedulable == true">
                            <a-tag  @click="setschedulable(record,false)"  color="#33CC00"  >开启调度</a-tag>
                        </template>
                        <template v-if="record.spec.unschedulable != true">
                            <a-tag  @click="setschedulable(record,true)"  color="#FF6600"  >关闭调度</a-tag>
                        </template>
                    </template>
                </template>
            </a-table>
...
export default({
    ...
    setup() {
        ...
        //修改node调度状态 ====================================EOF
        const setschedulableData = reactive({
            url : common.k8sNodeScheduling,
            params: {
                cluster: '',
                nodename: '',
                value: ''
        ...
        function setschedulable(record,b) {
            setschedulableData.params.cluster = localStorage.getItem('k8s_cluster')
            setschedulableData.params.nodename = record.metadata.name
            setschedulableData.params.value = b
            httpClient.get(setschedulableData.url, {params: setschedulableData.params})
            .then(res => {
                //响应成功，获取pod列表和total
        ...
        function nodeInfoUrl(e) {
            localStorage.setItem('nodename', e.metadata.name)
            router.push("/cluster/node/info?nodename="+e.metadata.name);
        }
        //EOF
